Shader "Simple Toon/SToon Outline Color Mask"
{
	Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			fixed4 frag (v2f i) : SV_Target
            {
				...
				fixed4 colByMask = i.color.r * _ColorR + i.color.g * _ColorG + i.color.b * _ColorB;
				colByMask.rgb = colByMask.rgb * _Tint;
				fixed4 litcol = ColorBlend(colByMask, _LightColor0, _AmbientCol);
        ...
        Pass
        {
			...
			fixed4 frag (v2f i) : SV_Target
            {
				...
				fixed4 shadecol = _DarkColor;
				fixed4 litcol = ColorBlend(_Tint, _LightColor0, _AmbientCol);
				fixed4 texcol = tex2D(_MainTex, i.uv) * litcol * _ColIntense + _ColBright;
		...
		Pass
        {
            ...
            v2f vert (appdata v)
            {
                v2f o;
			    o.pos = v.vertex;
			    float3 outlineVec = _UseVertexColor ? v.color.xyz : v.normal;
			    o.pos.xyz += normalize(outlineVec) * _OtlWidth * 0.008;
