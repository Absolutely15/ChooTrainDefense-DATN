...
namespace ATSoft.Ads
{
    public class Advertisements : Singleton<Advertisements>, IService
    {
        ...
        [SerializeField] private ATPublisher publisher = ATPublisher.InHouse;
        ...
        public ATPublisher Pulisher => publisher;
        public SupportedAdvertisers TypeAdvertiser => typeAdvertiser;
        ...
        private static bool CanShowConsent
        {
            get => PlayerPrefs.GetInt(CAN_SHOW_CONSENT, 0) == 0;
            set
            {
                PlayerPrefs.SetInt(CAN_SHOW_CONSENT, value ? 0 : 1);
                PlayerPrefs.Save();
            }
        }
        ...
        public void Initialize()
        {
            ...
            if (initialized == false)
            {
                ...
                // if (enableConsentFlow && CanShowConsent)
                // {
        ...
        {
            selectedAdvertiser?.ContinueConsentFlow();
        }
        ...
        private void ContinueInitialization()
        {
...
#if AD_TYPE_IS
                    var isComp = gameObject.AddComponent<CustomIronSource>();
                    var tempAdvertiserIS =
                        advertisers.FirstOrDefault(condition => condition.advertiser == SupportedAdvertisers.IronSource);
                    if (tempAdvertiserIS != null)
                        advertiser =
                            new Advertiser(isComp, SupportedAdvertisers.IronSource,
                                tempAdvertiserIS.advertiserSettings);
                    break;
#endif
                default:
            ...
            selectedAdvertiser?.InitializeAds(advertiser.advertiserSettings, isTestAds, enableDebugger, enableConsentFlow);
            initialized = true;
        }
        ...
        public void ShowInterstitial(UnityAction InterstitialClosed = null, string Placement = "",
            bool autoDisableLoading = true, bool enableLoadingCanvas = true)
        {
            if (!SdkAutoInitializer.Instance.enableAds)
            {
            ...
            //todo
            var currentLevel = PlayerSave.CurrentGameLevel + 1;
            if (currentLevel < FirebaseRemoteConfigManager.interstitialStartLevel)
            {
                InterstitialClosed?.Invoke();
                return;
            }

            if (selectedAdvertiser == null) return;
            
            Debug.Log("Interstitial loaded from " +  selectedAdvertiser);

            var showSuccess =
                selectedAdvertiser.ShowInterstitial(InterstitialClosed, Placement, autoDisableLoading, enableLoadingCanvas);
            //LogEvent
#if PUB_RK //Rocket
                ...
                //if(showSuccess) AnalyticsManager.LogEventImpdauInterPassed();
#else //In-house
            AnalyticsManager.LogEventShowInterstitial(showSuccess, Placement);
            if (showSuccess) AnalyticsManager.LogEventImpdauInterPassed();
#endif
        }
        ...
        {
            //if ads are disabled by user -> do nothing
            if (!SdkAutoInitializer.Instance.enableAds || CanShowAds() == false)
            {
                return;
            }
